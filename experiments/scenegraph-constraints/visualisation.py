...
with open("models/scenegraph-constraints.model", "rb") as f:
    model = pickle.load(f)
    
step = 5
X = [[sys.argv[2], sys.argv[1], (x - 300)/100, (y - 300)/100] for x in range(0, w, step) for y in range(0, h, step)]
y = model.predict(X)

grid = np.reshape(y, (w // step, h // step))
        
obj = {}
obj["drawing"] = quickdraw.get_quickdraw_drawing(sys.argv[2], 1)
...
overlay_draw.text((10, 10), sys.argv[1] + " " + sys.argv[2], font=ImageFont.truetype("Verdana.ttf",20), fill="black")
